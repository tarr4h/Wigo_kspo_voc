...
public class FileInfoTag extends TagSupport {
	...
	public int doStartTag() throws JspException {
		...
		String dataInfo = "data-file-id='" + fileId + "' data-file-seq='" + fileSeq + "' data-file-category='"
				+ fileCategory + "' data-accept-ext='" + acceptExts + "' data-accept-types='" + acceptFileTypes + "' "
				+ idNm;
		StringBuffer info = new StringBuffer();
		info.append("<span ").append(dataInfo).append(" data-file-info='info' >");
		String button = "&nbsp;<button " + dataInfo
				+ " class=\"btnInner gray btnSubmit\" type=\"button\" data-click=\"onFileInfo\" data-file-info='button'>첨부</button>";
		String cancel = "&nbsp;<button " + dataInfo
				+ " class=\"btnInner gray btnSubmit\" style='display:none'  type=\"button\" data-click=\"onCancelFileInfo\"  data-file-info='cancel'>취소</button>";
		String remove = "&nbsp;<button " + dataInfo
				+ " class=\"btnInner gray btnSubmit\" style='display:none'  type=\"button\" data-click=\"onRemoveFileInfo\"  data-file-info='remove'>삭제</button>";
		String download = "&nbsp;<button " + dataInfo
				+ " style='display:none' class=\"btnInner gray btnSubmit\" data-file-url='' type=\"button\" data-click=\"onDownloadFileInfo\"  data-file-info='download'>받기</button>";
		...
		try {
			...
			if (Utilities.isEmpty(fileList))
				...
				remove = "&nbsp;<button " + dataInfo
						+ " class=\"btnInner gray btnSubmit\" type=\"button\" data-click=\"onRemoveFileInfo\"  data-file-info='remove'>삭제</button>";
				download = "&nbsp;<button " + dataInfo + " class=\"btnInner gray btnSubmit\" data-file-url='"
						+ fileList.get(0).getFileUrl() + "' data-file-nm='" + fileList.get(0).getFileNm()
						+ "' type=\"button\" data-click=\"onDownloadFileInfo\"  data-file-info='download'>받기</button>";
