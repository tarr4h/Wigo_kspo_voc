...
*************************************************************/
package com.egov.voc.base.common.util.security;
			...
			if (pwdByte.length <= TEXT_SIZE) {
				...
				for (int i = 0; i < loop; i++) {
					if ((pwdByte.length - end) <= TEXT_SIZE) {
						...
						temp = seedEncrypt (addAnsiX923Padding (new String (pwdByte, i * TEXT_SIZE, period,_ENC_LANG).getBytes (_ENC_LANG), TEXT_SIZE),key);
						for (int j = 0; j < temp.length; j++)
							result[(i * TEXT_SIZE) + j] = temp[j];
					} else {
						temp = seedEncrypt (new String (pwdByte, i * TEXT_SIZE, period,_ENC_LANG).getBytes(_ENC_LANG),key);
